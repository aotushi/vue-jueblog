...
const toPublish = () => {
  ...
  if (form.value.status == 1) {
    article_store.updateArticle(mode.value, form.value, () => {
      loading.value = false
      location.href = '/article/' + mode.value
    })
  } else {
    article_store.publishArticle(mode.value, () => {
      loading.value = false
      location.href = '/article/' + mode.value
    })
  }
...
const ctxChange = debounce((key: 'title' | 'content') => {
  ...
  if (mode.value == 'create' && form.value[key]) {
    loading.value = true
    article_store.createArticle(form.value, (res: mdArticleType) => {
      mode.value = res._id
...
onMounted(() => {
  ...
  if (tag != 'create') {
    article_store.getArticlesDetail(tag, data => {
      // article.value = data
      form.value = data
    })
  }
          ...
          <el-form label-width="85px">
            <el-form-item required label="分类：">
              <div
                ...
                v-for="item in article_store.articleInfo.categories"
