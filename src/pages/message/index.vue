...
const toFollow = (row: FollowType['data'][0]) => {
  let { fans_id } = row
  toggleFollow({ user_id: fans_id }, () => {
...
const getMessage = () => {
  ...
  if (type.value == '2') {
    msgStore.getPraises(res => {
      ...
      praises.value =
        page == 1
          ? (res.data as unknown as PraiseType[])
          : ([...praises.value, ...res.data] as unknown as PraiseType[])
      meta.value = res.meta as unknown as {
        page: number
        per_page: number
        total: number
      }
