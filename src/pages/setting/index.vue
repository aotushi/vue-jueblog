...
const uploadSuccess = (file: { code: number; data: { path: string } }) => {
  if (file.code == 200) {
    form.value!.avatar = 'https://localhost:9000/' + file.data.path
...
onMounted(() => {
  ...
  if (!store.user_state.user_info) {
    return (location.href = '/')
  }
  console.log(
    'setting>onMounted>store.user_state.user_info',
    store.user_state.user_info,
  )

  form.value = store.user_state.user_info
  curtag.value = tag
})
</script>
...
<template>
  <section class="setting-page fxt">
    ...
    <div class="sother panel fxt">
      ...
      <div v-if="curtag == 'drafts'">草稿箱未开放</div>
    </div>
  </section>
</template>
