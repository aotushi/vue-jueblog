...
// GET /list/:id — 获取评论列表（前端调用路径）
comments.get('/list/:id', async c => {
...
// POST /create — 创建评论（前端调用路径，需登录）
comments.post('/create', authMiddleware, async c => {
  const {
    source_id,
    type = 'article',
    content,
...
// DELETE /delete/:id — 删除评论（前端调用路径，需登录）
comments.delete('/delete/:id', authMiddleware, async c => {
  const id = Number(c.req.param('id'))
  const userId = c.get('userId')
  const db = c.env.DB
  const existing = await db
